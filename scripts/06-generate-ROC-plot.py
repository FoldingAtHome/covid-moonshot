...
if __name__ == '__main__':
    ...
    for subset in molecules:
        molecule_subset = molecules[subset]
        print(f'{subset} has {len(molecule_subset)} molecules')

        # Compute ROC curve
        print('Computing ROC curve...')
        try:
            fpr, tpr, AUC = compute_ROC(molecule_subset)
        except Exception as e:
            print(e)
            continue

        # Bootstrap AUC
        print(f'Bootstrapping AUC...')
        AUC_low, AUC_high, AUC_std = bootstrap_AUC(molecule_subset)
        ...
        fig = plt.figure()
        plt.plot(fpr, tpr, 'k-')
        plt.title(f'{subset} : AUC = {AUC:0.3} (95% CI: [{AUC_low:0.3}, {AUC_high:0.3}]; N = {len(molecule_subset)})')
        plt.xlabel('false positive rate')
        plt.ylabel('true positive rate')
        plt.tight_layout()
        fig.savefig(f'ROC - {subset}.png')
